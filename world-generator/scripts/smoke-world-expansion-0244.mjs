...
for(const id of ['iberia','france','canarias','northwest-africa']){
  const c=cfg(id);assert.equal(c.geography.scale.horizontalUnitsPerKm,SCALE,`${id} scale drift`);assert.ok(tri(c)<=c.performance.budgets.visibleTriangles,`${id} terrain budget exceeded`);
  ...
  const dir=path.join(ROOT,`regions/${id}`);if(fs.existsSync(path.join(dir,'manifest.json'))){
    const m=read(`regions/${id}/manifest.json`);assert.equal(m.region.id,id);assert.equal(m.projection.unitsPerKm,SCALE,`${id} manifest scale drift`);const minPeak=id==='canarias'?1000:id==='northwest-africa'?3500:2000;assert.ok(m.terrain.maximumElevationMeters>minPeak,`${id} relief peak too low`);assert.ok(m.content.settlements>0,`${id} settlement layer missing`);
  }
}
...
console.log(JSON.stringify({ok:true,scale:SCALE,triangles:{iberia:tri(cfg('iberia')),france:tri(cfg('france')),canarias:tri(cfg('canarias')),'northwest-africa':tri(cfg('northwest-africa'))}},null,2));