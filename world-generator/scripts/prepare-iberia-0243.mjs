...
const alignOld="if(state.adventureFlight){const blend=Math.min(1,dt*5.5),targetPitch=Math.max(-.42,Math.min(.30,-state.adventureFlightVy*.07));state.terrainPitch+=(targetPitch-state.terrainPitch)*blend;state.terrainRoll+=(state.adventureFlightBank-state.terrainRoll)*blend;state.slopeAngle=0;return;}";
const align0243="if(state.adventureFlight){const bearded=window.__WAFT_INTERNAL_GAME__?.mountedAnimalId==='iberia-bearded-vulture',blend=Math.min(1,dt*(bearded&&state.iberiaDive?10:5.5)),targetPitch=bearded&&state.iberiaDive?.80:Math.max(-.50,Math.min(.42,-state.adventureFlightVy*.07));state.terrainPitch+=(targetPitch-state.terrainPitch)*blend;state.terrainRoll+=(state.adventureFlightBank-state.terrainRoll)*blend;state.slopeAngle=0;return;}";
...
patch('mallorca-mobile/adventure-0210/index.html',[
  ...
  [alignOld,align0243,'quebrantahuesos dive attitude'],
  [
...
patch('mallorca-mobile/adventure-0210/plugin-loader.js',[
  [
    ...
    "PenínsULA IBÉRICA · EXPLORACIÓN 0.24.3 · guardados ".replace('PenínsULA','Península'),
    'Iberia HUD version'
  ]
], 'EXPLORACIÓN 0.24.3');
...
// Keep an already-generated 0.24.3 runtime upgradable when flight tuning changes inside the same release.
{
  ...
  const oldAlignCurrent="if(state.adventureFlight){const blend=Math.min(1,dt*5.5),targetPitch=Math.max(-.42,Math.min(.30,-state.adventureFlightVy*.07));state.terrainPitch+=(targetPitch-state.terrainPitch)*blend;state.terrainRoll+=(state.adventureFlightBank-state.terrainRoll)*blend;state.slopeAngle=0;return;}";
  ...
  if(source.includes(oldAlignCurrent)){source=source.replace(oldAlignCurrent,align0243);changed=true;}
  if(changed){fs.writeFileSync(file,source);console.log('mallorca-mobile/adventure-0210/index.html: upgraded 0.24.3 flight tuning');}
}
