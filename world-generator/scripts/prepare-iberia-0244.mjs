...
patch('mallorca-mobile/adventure-0210/index.html',[
  ...
  [flight0243,flight0244,'short strong bearded-vulture climb impulse'],
...
// Same-release flight tuning: preserve the stronger kick but shorten its momentum so neutral input levels immediately again.
{
  const file=path.join(ROOT,'mallorca-mobile/adventure-0210/index.html');
  let source=fs.readFileSync(file,'utf8'),changed=false;
  if(source.includes(flap0244Long)){source=source.replace(flap0244Long,flap0244);changed=true;}
  if(source.includes(flight0244Long)){source=source.replace(flight0244Long,flight0244);changed=true;}
  if(changed){fs.writeFileSync(file,source);console.log('mallorca-mobile/adventure-0210/index.html: shortened 0.24.4 climb momentum');}
}
