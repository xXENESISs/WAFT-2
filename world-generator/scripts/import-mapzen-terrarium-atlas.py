#!/usr/bin/env python3
"""Build the WAFT 0.25.2 continuous Europe atlas DEM.
...
def fetch(url,target,attempts=4):
    ...
    for i in range(attempts):
        try:
            req=urllib.request.Request(url,headers={"User-Agent":"WAFT-Europe-Atlas/0.25.2"})
...
def main():
    p=argparse.ArgumentParser();p.add_argument("region_id",nargs="?",default="europe-atlas");p.add_argument("--root",default=None);p.add_argument("--refresh",action="store_true");a=p.parse_args()
