...
(async()=>{
  ...
  atlanticMesh=null; // WAFT 0.26.0: no artificial Atlantic corridor geometry
  ...
  const sampleAtlantic=()=>null; // WAFT 0.26.0: no artificial Atlantic corridor surface
  ...
  const updateWorldState=()=>{
    ...
    if(african){regionBadge.hidden=true;if(hud)hud.textContent='NOROESTE DE ÁFRICA · MUNDO CONTINUO 0.26.0';}
    ...
    else{regionBadge.hidden=true;if(hud&&(hud.textContent==='FRANCE · MONDE CONTINU'||hud.textContent==='FRANCE 001 · MONDE CONTINU'||hud.textContent==='CANARIAS · MUNDO CONTINUO'))hud.textContent='PENÍNSULA IBÉRICA · EXPLORACIÓN 0.26.0';}
