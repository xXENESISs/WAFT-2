...
for(const test of [
  ...
  for(const pattern of [
    ...
    /plugin-loader\.js/,
    /__WAFT_ADVENTURE_BUILD__='0\.25\.0'/,
  ...
  console.log(`${test.id}: optimized patched 0.25.0 runtime compiled (${written.length} chars)`);
}
console.log('Both existing World 2 regional runtimes survive the 0.25.0 bootstrap while retaining spatial building queries, adaptive movement/camera probes, UI close paths, GPU streaming hooks and World 1 parity.');