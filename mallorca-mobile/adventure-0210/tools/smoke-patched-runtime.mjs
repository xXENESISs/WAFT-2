...
for(const test of [
  ...
  console.log(`${test.id}: optimized patched 0.24.6 runtime compiled (${written.length} chars)`);
}
console.log('Both existing World 2 regional runtimes survive the 0.24.6 bootstrap while retaining spatial building queries, adaptive movement/camera probes, UI close paths, GPU streaming hooks and World 1 parity.');