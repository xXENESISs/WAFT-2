...
(() => {
  ...
  function visible(ctx){
    ...
    const state=api.getState?.(),pd=state?.position;
  ...
  function drawAnimatedGoat(ctx){
    ...
    drawCylinderPart(r,base,-.18,1.25,.91,.045,.38,.045,horn,M.rz(-.42));drawCylinderPart(r,base,.18,1.25,.91,.045,.38,.045,horn,M.rz(.42));drawSphere(r,base,0,.66,1.19,.08,.18,.07,[.30,.24,.19]);
  }

  function drawAnimatedCow(ctx){
    const {r,a,now,drawSphere,drawCylinderPart,M}=ctx;
    const phase=now*.0042+a.phase,stride=Math.sin(phase)*.22,bounce=Math.abs(Math.sin(phase*2))*.018,base=M.compose(ctx.base,M.t(0,bounce,0));
    const hide=[.52,.22,.12],dark=[.17,.10,.07],cream=[.73,.61,.45],horn=[.74,.69,.57];
    drawSphere(r,base,0,.72,-.05,1.05,.56,1.28,hide);drawSphere(r,base,0,1.08,1.08,.43,.37,.43,hide);drawSphere(r,base,0,.98,1.48,.34,.24,.30,cream);
    ...
    drawCylinderPart(r,base,-.25,1.37,1.20,.035,.25,.035,horn,M.rz(-.55));drawCylinderPart(r,base,.25,1.37,1.20,.035,.25,.035,horn,M.rz(.55));
  }
  ...
  function installRenderer(){
    ...
    upgraded.__waft0233=true;window.WAFTAnimalRenderer0230=upgraded;return true;
  }

  async function init(){
    for(let i=0;i<500;i++){if(window.__WAFT_INTERNAL_GAME__&&window.WAFTRegionRuntime&&window.WAFTAnimalRenderer0230)break;await new Promise(resolve=>setTimeout(resolve,20));}
    normalizeMounts();installRenderer();
    const timer=setInterval(()=>{normalizeMounts();installRenderer();},900);addEventListener('pagehide',()=>clearInterval(timer),{once:true});
    window.WAFTParity0233={version:BUILD,normalizeMounts,installRenderer};window.__WAFT_PARITY_0233_READY__=true;
  }
