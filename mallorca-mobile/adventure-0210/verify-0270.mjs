...
need(browserVerifier.includes('const runnerBaseline=await frameTrace();')&&browserVerifier.includes('FRAME_TRACE_GENERATION__!==generation')&&browserVerifier.includes("const allowSaturatedRunner=process.env.WAFT_ALLOW_SLOW_RUNNER==='1';")&&browserVerifier.includes('establishMountedSceneBaseline')&&browserVerifier.includes('movement or camera added frame spikes beyond the stationary runner scene'),'hosted-runner frame calibration is missing');
