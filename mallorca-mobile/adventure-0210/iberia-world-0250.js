'use strict';
(async()=>{
  if(window.__WAFT_IBERIA_WORLD_0250_READY__||window.__WAFT_ADVENTURE_REGION__!=='iberia')return;
  ...
  if(!api||!stream||!continuity||!plugin)throw new Error('WAFT 0.25.0 runtime unavailable');
  const canvas=document.querySelector('canvas'),gl=canvas?.getContext('webgl2');
  if(!gl)throw new Error('WebGL2 unavailable for WAFT 0.25.0');
  ...
  const updateLabels=()=>{
    ...
    const hud=document.getElementById('hudTitle');if(hud&&inAfrica(g))hud.textContent='NOROESTE DE ÁFRICA · MUNDO CONTINUO 0.25.0';
  };
  setInterval(updateLabels,120);updateLabels();

  window.WAFTWorld0250={version:'0.25.0',inAfrica,inAtlantic,prefetchAfrica,sampleAfrica,sampleOcean,getState:()=>({phase:state.phase,scale:U,africaReady:Boolean(state.mesh),africaTriangles:state.mesh?.triangles||0,africaDrawFrames:state.drawFrames,oceanTriangles:state.ocean?.triangles||0,oceanDrawFrames:state.oceanFrames,africaBytes:state.bytes,africaSettlements:state.settlements.length,shownLabels:state.shownLabels,nearest:state.nearest?.name||null,labelRangeKm:LABEL_RANGE_KM,nearestRangeKm:NEAREST_RANGE_KM,labelMaxAglM:LABEL_MAX_AGL_M,geo:state.lastGeo,error:state.error})};
  window.__WAFT_IBERIA_WORLD_0250_READY__=true;
})().catch(e=>{console.error('WAFT 0.25.0 failed',e);window.__WAFT_IBERIA_WORLD_0250_ERROR__=String(e?.message||e);});
