...
(async () => {
  ...
  if(terrainOnly){
    ...
    replaceOne("    progress.textContent = REGION_NAMES[REGION_ID] + ' · fauna ' + observedCount + '/' + game.animals.length + ' · guardados ' + game.saveCount;","    progress.textContent = REGION_ID === 'iberia' ? 'Península Ibérica · EXPLORACIÓN 0.24.2 · guardados ' + game.saveCount : REGION_NAMES[REGION_ID] + ' · fauna ' + observedCount + '/' + game.animals.length + ' · guardados ' + game.saveCount;",'HUD terreno Iberia');
