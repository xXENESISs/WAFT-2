...
(async()=>{
  ...
  const desiredHud=state=>{
    ...
    if(!state?.position)return version250?'PENÍNSULA IBÉRICA · EXPLORACIÓN 0.26.0':'PENÍNSULA IBÉRICA · EXPLORACIÓN 0.24.9';
    const g=geoFromWorld(state.position.x,state.position.z);
    if(world250?.inAfrica?.(g))return'NOROESTE DE ÁFRICA · MUNDO CONTINUO 0.26.0';
    if(continuity.inCanarias?.(g))return'CANARIAS · MUNDO CONTINUO';
    if(continuity.inFrance?.(g))return'FRANCE · MONDE CONTINU';
    return version250?'PENÍNSULA IBÉRICA · EXPLORACIÓN 0.26.0':'PENÍNSULA IBÉRICA · EXPLORACIÓN 0.24.9';
  };
