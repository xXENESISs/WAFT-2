...
(async()=>{
  ...
  const state={ready:false,phase:'boot',originGeo:legacyGeo(start.x,start.z),terrain:null,cover:null,europeTerrain:null,europeCover:null,mesh:null,cityMesh:null,objects:[],drawFrames:0,triangles:0,visibleCities:0,patchRebuilds:0,floatingOriginShifts:0,poleCrossings:0,datelineCrossings:0,speedEstimate:0,prefetchLead:180,lastGeo:null,lastFrameAt:performance.now(),lastBuildPlayer:{x:Infinity,z:Infinity},lastBuildHeading:0,lastBuildAt:0,error:null};
  ...
  function maybeRecenter(){const s=api.getState?.(),pos=s?.position;if(!pos||Math.hypot(pos.x,pos.z)<RECENTER)return false;const oldOrigin={...state.originGeo},newGeo=geoFromLocal(pos.x,pos.z),h=Number(s.playerFacing)||0,forwardGeo=geoFromLocal(pos.x+Math.sin(h)*2,pos.z+Math.cos(h)*2),geoBearing=bearing(newGeo,forwardGeo),newHeading=Math.atan2(Math.sin(Math.PI-geoBearing),Math.cos(Math.PI-geoBearing));const oldLon=oldOrigin.lon,oldLat=oldOrigin.lat;state.originGeo=newGeo;if(Math.abs(wrapLon(newGeo.lon-oldLon))>150)state.datelineCrossings++;if(Math.sign(oldLat)!==Math.sign(newGeo.lat)&&Math.abs(oldLat)>75&&Math.abs(newGeo.lat)>75)state.poleCrossings++;api.setRegionalPosition?.(0,0,pos.y);api.setHeading?.(newHeading);state.floatingOriginShifts++;state.lastBuildPlayer={x:Infinity,z:Infinity};rebuildPatch(true);return true;}

  function updateSpeed(now){const s=api.getState?.(),pos=s?.position;if(!pos)return;const g=geoFromLocal(pos.x,pos.z);if(state.lastGeo){const dt=Math.max(.001,(now-state.lastFrameAt)/1000),instant=haversineKm(state.lastGeo,g)*U/dt;state.speedEstimate+= (clamp(instant,0,160)-state.speedEstimate)*(1-Math.exp(-dt*3.5));}state.lastGeo=g;state.lastFrameAt=now;state.prefetchLead=clamp(state.speedEstimate*6,180,700);}
